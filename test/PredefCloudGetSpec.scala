...
class PredefCloudGetSpec extends Specification {
  def is =
    "PredefCloudGetSpec".title ^ end ^
      """
PredefCloudGetSpec is the implementation that calls the megam_play API server with the /predefclouds url to fetch predef's
    """ ^ end ^
      "The Client Should" ^
  ...
  trait Context extends BaseContextGet {
    ...
    protected lazy val url = new URL("http://localhost:9000/v1/predefclouds")
