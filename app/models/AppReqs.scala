 ...
 */
case class AppRequestInput(req_type: String, node_name: String, appdefns_id: String, lc_apply: String, lc_additional: String, lc_when: String) {
  val json = "\"node_name\":\"" + node_name + "\",\"appdefns_id\":\"" + appdefns_id + "\",\"req_type\":\"" + req_type + "\",\"lc_apply\":\"" + lc_apply + "\",\"lc_additional\":\"" + lc_additional + "\",\"lc_when\":\"" + lc_when + "\""
}

case class AppRequestResult(id: String, node_id: String, node_name: String, appdefns_id: String, req_type: String, lc_apply: String, lc_additional: String, lc_when: String, created_at: String) {
  val json = "{\"id\": \"" + id + "\",\"node_id\": \"" + node_id + "\",\"node_name\":\"" + node_name + "\",\"appdefns_id\":\"" + appdefns_id + "\",\"req_type\":\"" + req_type + "\",\"lc_apply\":\"" + lc_apply + "\",\"lc_additional\":\"" + lc_additional +
...
object AppRequestResult {
  ...
  def apply = new AppRequestResult(new String(), new String(), new String(), new String(), new String(), new String(), new String(), new String(), new String())
  ...
  def fromJson(json: String): Result[AppRequestResult] = (Validation.fromTryCatch[net.liftweb.json.JValue] {
    parse(json)
  } leftMap { t: Throwable =>
...
object AppRequests {
  ...
  private def mkGunnySack(input: String): ValidationNel[Throwable, Option[GunnySack]] = {
    ...
    val ripNel: ValidationNel[Throwable, AppRequestInput] = (Validation.fromTryCatch[models.AppRequestInput] {
    ...
    for {
      ...
      val tuple_node = aor.list filter (nelwop => nelwop.isDefined) map { nelnor: Option[NodeResult] =>
        (if (nelnor.isDefined) { //we only want to use the Some, ignore None. Hence a pattern match wasn't used here.
          Tuple2(nelnor.get.id, nelnor.get.appdefnsid)
        }).asInstanceOf[Tuple2[String, String]]
      }      
      val bvalue = Set(tuple_node(0)._1)
      val appdefns_bvalue = Set(rip.appdefns_id)
      val json = "{\"id\": \"" + (uir.get._1 + uir.get._2) + "\",\"node_id\":\"" + tuple_node(0)._1 + "\"," + rip.json + ",\"created_at\":\"" + Time.now.toString + "\"}"
      new GunnySack((uir.get._1 + uir.get._2), json, RiakConstants.CTYPE_TEXT_UTF8, None,
        Map(metadataKey -> newnode_metadataVal), Map((newnode_bindex, bvalue),(appdefns_bindex, appdefns_bvalue))).some
    }    
  }  
   ...
   */
  def findByReqName(reqNameList: Option[List[String]]): ValidationNel[Error, AppRequestResults] = {
    play.api.Logger.debug(("%-20s -->[%s]").format("models.AppRequest", "findByReqName:Entry"))
    ...
    (reqNameList map {
      _.map { reqName =>
        ...
        (riak.fetch(reqName) leftMap { t: NonEmptyList[Throwable] =>
          ...
          xso match {
            case Some(xs) => {
              (Validation.fromTryCatch[models.AppRequestResult] {
                parse(xs.value).extract[AppRequestResult]
              } leftMap { t: Throwable =>
                new ResourceItemNotFound(reqName, t.getMessage)
              }).toValidationNel.flatMap { j: AppRequestResult =>
                Validation.success[Error, AppRequestResults](nels(j.some)).toValidationNel  
              }
            }
            case None => Validation.failure[Error, AppRequestResults](new ResourceItemNotFound(reqName, "")).toValidationNel
          }
        }
      } // -> VNel -> fold by using an accumulator or successNel of empty. +++ => VNel1 + VNel2
    } map {
      _.foldRight((AppRequestResults.empty).successNel[Error])(_ +++ _)
   ...
   * Using a "nodename" as key, return a list of ValidationNel[List[AppRequestResult]] 
   * Takes an email, and returns a Future[ValidationNel, List[Option[AppRequestResult]]]
   */
  def findByNodeName(nodeNameList: Option[List[String]]): ValidationNel[Throwable, AppRequestResults] = {
    play.api.Logger.debug(("%-20s -->[%s]").format("models.AppRequest", "findByNodeName:Entry"))
    play.api.Logger.debug(("%-20s -->[%s]").format("nodeNameList", nodeNameList))
    val res = eitherT[IO, NonEmptyList[Throwable], ValidationNel[Error, AppRequestResults]] {
      ((((for {
        ...
        (if (!nm.isEmpty) findByReqName(nm.some) else
          new ResourceItemNotFound(nodeNameList.map(m => m.mkString("[", ",", "]")).get, "application requests = nothing found.").failureNel[AppRequestResults])
      }).disjunction).pure[IO]
    }.run.map(_.validation).unsafePerformIO
    res.getOrElse(new ResourceItemNotFound(nodeNameList.map(m => m.mkString("[", ",", "]")).get, "application requests = nothing found.").failureNel[AppRequestResults])
