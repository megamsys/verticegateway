...
object CloudTool {
  ...
  val opennebula = CloudTemplate("opennebula", CloudInstructionGroup(List("server" -> CloudInstructions(
    CloudInstruction("create", "server create -c", "-N"),
    CloudInstruction("delete", "server delete <node_name> -c -P -y", "-N"),
    CloudInstruction("list", "server list", "")), "instance" -> CloudInstructions(
    CloudInstruction("data", "instance set", "-N")))))
  ...
  val cloudtemplates = CloudTemplates(ec2, rackspace, openstack, hp, gce, profitbricks, opennebula, gogrid)
