...
//      parse(input).extract[Component]
 //   } leftMap { t: Throwable => new MalformedBodyError(input, t.getMessage) }).toValidationNel //capture failure
    ...
    for {  
      ...
      val json = "{\"id\": \"" + (uir.get._1 + uir.get._2) + "\",\"name\":\"" + input.name + "\",\"tosca_type\":\"" + input.tosca_type + "\",\"requirements\":\"" + input.requirements + "\",\"inputs\":" + input.inputs + "\",\"external_management_resource\":" + input.external_management_resource + "\",\"artifacts\":\"" + input.artifacts + "\",\"related_components\":\"" + input.related_components + "\",\"operations\":\"" + input.operations + "\",\"created_at\":\"" + Time.now.toString + "\"}"
      new GunnySack((uir.get._1 + uir.get._2), json, RiakConstants.CTYPE_TEXT_UTF8, None,
        Map(metadataKey -> metadataVal), Map((bindex, bvalue))).some   
    }
  }

  def createLinks(email: String, input: models.tosca.Components): List[ValidationNel[Throwable, String]] = {
    play.api.Logger.debug(("%-20s -->[%s]").format("models.ComponentLinks", "createMany:Entry"))
    play.api.Logger.debug(("%-20s -->[%s]").format("json", input))

//    val componentInput: ValidationNel[Throwable, Components] = (Validation.fromTryCatch[models.tosca.Components] {
 //     parse(input).extract[]
 //   } leftMap { t: Throwable => new MalformedBodyError(input, t.getMessage) }).toValidationNel //capture failure
   ...
   */
  def create(email: String, input: Component): ValidationNel[Throwable, String] = {
    play.api.Logger.debug(("%-20s -->[%s]").format("tosca.Component", "create:Entry"))
    play.api.Logger.debug(("%-20s -->[%s]").format("yaml", input))

    (mkGunnySack(email, input) leftMap { err: NonEmptyList[Throwable] =>
      new ServiceUnavailableError(input.json, (err.list.map(m => m.getMessage)).mkString("\n"))
    }).toValidationNel.flatMap { gs: Option[GunnySack] =>

      (riak.store(gs.get) leftMap { t: NonEmptyList[Throwable] => t }).
        flatMap { maybeGS: Option[GunnySack] =>
          maybeGS match {
            case Some(thatGS) => println(thatGS.value); ((thatGS.value).some).successNel[Throwable]
            case None => {
              play.api.Logger.warn(("%-20s -->[%s]").format("Component.created success", "Scaliak returned => None. Thats OK."))
