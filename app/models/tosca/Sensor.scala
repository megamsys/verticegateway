...
case class SensorInput( sensor_type: String, payload: Payload) {
  val json = "{\"sensor_type\":\"" + sensor_type + "\",\"payload\":" + payload.json + "}"

}

case class SensorResult(id: String, sensor_type: String, payload: Payload, created_at: String) {
...
case class Sensor(sensor_type: String, payload: Payload) {
  val json = "{\"sensor_type\":\"" + sensor_type + "\",\"payload\":" + payload.json + "}"
...
object Sensor {
  ...
  def empty: Sensor = new Sensor(new String(), Payload.empty)
   ...
   */
   private def mkGunnySack(email: String, input: Sensor, asm_id: String): ValidationNel[Throwable, Option[GunnySack]] = {
     ...
     for {
       ...
       val bvalue = Set(aor.get.id)
       val json = "{\"id\": \"" + (uir.get._1 + uir.get._2) + "\", \"Sensor_type\":\"" + input.sensor_type + "\",\"Payload\":" + input.payload.json + "\",\"created_at\":\"" + Time.now.toString + "\"}"
