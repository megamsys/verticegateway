...
object CSARs extends Controller with APIAuthElement {
  ...
  def post = StackAction(parse.tolerantText) { implicit request =>
    play.api.Logger.debug(("%-20s -->[%s]").format("controllers.CSAR", "post:Entry"))
    ...
    (Validation.fromTryCatch[Result] {
      reqFunneled match {
        case Success(succ) => {
          ...
          play.api.Logger.debug(("%-20s -->[%s]").format("controllers.CSAR", "request funneled."))
  ...
  def show(id: String) = StackAction(parse.tolerantText) { implicit request =>
    play.api.Logger.debug(("%-20s -->[%s]").format("controllers.CSAR", "show:Entry"))
  ...
  def list = StackAction(parse.tolerantText) { implicit request =>
    play.api.Logger.debug(("%-20s -->[%s]").format("controllers.CSAR", "list:Entry"))
