...
object Snapshots extends Controller with APIAuthElement with PermissionElement {
    ...
    (Validation.fromTryCatchThrowable[Result, Throwable] {
      reqFunneled match {
        case Success(succ) => {
          ...
          models.admin.Snapshots.list match {
            case Success(succ) => {
              Ok(Results.resultset(models.Constants.SNAPSHOTSCOLLECTIONCLAZ, compactRender(Extraction.decompose(succ))))
             }
            case Failure(err) =>
