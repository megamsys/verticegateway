 ...
 *
 */
object Flavors extends Controller with controllers.stack.APIAuthElement {
    ...
    (Validation.fromTryCatchThrowable[Result,Throwable] {
      reqFunneled match {
        case Success(succ) => {
          ...
          models.admin.Flavors.create(email, clientAPIBody) match {
            case Success(succ) =>
            Status(CREATED)(
              FunnelResponse(CREATED, """[%s]  submitted successfully.""".format(succ.get.id), FLAVORCLAZ).toJson(true)
          )
            case Failure(err) => {
  ...
  def showByName(name: String) = StackAction(parse.tolerantText) { implicit request =>
    (Validation.fromTryCatchThrowable[Result, Throwable] {
      reqFunneled match {
        case Success(succ) => {
          val freq = succ.getOrElse(throw new Error("Invalid header."))
          val email = freq.maybeEmail.getOrElse(throw new Error("Email not found (or) invalid."))
          models.admin.Flavors.findByName(name, email) match {
            case Success(succ) =>
    ...
    (Validation.fromTryCatchThrowable[Result, Throwable] {
      reqFunneled match {
        case Success(succ) => {
          ...
          models.admin.Flavors.update(email, clientAPIBody) match {
            case Success(succ) =>
              Status(CREATED)(
                FunnelResponse(CREATED, "Your marketplaces updated successfully.", FLAVORCLAZ).toJson(true))
            case Failure(err) =>
              val rn: FunnelResponse = new HttpReturningError(err)
              Status(rn.code)(rn.toJson(true))
          }
    ...
    (Validation.fromTryCatchThrowable[Result, Throwable] {
      reqFunneled match {
        case Success(succ) => {
          val freq = succ.getOrElse(throw new Error("Invalid header."))
          val email = freq.maybeEmail.getOrElse(throw new Error("Email not found (or) invalid."))
          models.admin.Flavors.delete(email, flavor) match {
            case Success(succ) =>
              Status(CREATED)(
                FunnelResponse(CREATED, flavor + "  deleted successfully.", FLAVORCLAZ).toJson(true))
            case Failure(err) =>
              val rn: FunnelResponse = new HttpReturningError(err)
              Status(rn.code)(rn.toJson(true))
          }
